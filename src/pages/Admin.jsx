...
export const Admin = () => {
  ...
  if (loadingUsr) {
    return <div className="min-h-screen bg-[#0a110a] flex items-center justify-center text-brand-500 font-bold">Cargando panel...</div>;
  }
    ...
    <>
      ...
      <div className="min-h-screen bg-[#0a110a] text-white flex flex-col md:flex-row pb-16 md:pb-0 pt-[80px]">
        ...
        <aside className="hidden md:flex flex-col w-64 bg-[#080d08]/80 backdrop-blur-2xl border-r border-white/5 p-6 shadow-[20px_0_30px_rgba(0,0,0,0.5)] z-40 relative">
          <div className="absolute top-0 left-0 w-full h-32 bg-brand-500 opacity-5 blur-[50px] pointer-events-none"></div>

          <div className="mb-10 relative z-10">
            <div className="flex items-center gap-3 mb-6">
              <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-brand-500 to-brand-800 shadow-[0_0_15px_rgba(124,82,40,0.4)] flex items-center justify-center border border-white/10">
                <span className="font-bold text-white tracking-widest">{config.nombreDelNegocio.charAt(0)}</span>
              </div>
              <div>
                <h1 className="text-xs font-black text-gray-300 uppercase tracking-widest leading-tight">{config.nombreDelNegocio}</h1>
                <h2 className="text-xs font-black text-brand-500 uppercase tracking-widest leading-tight">Admin Panel</h2>
              </div>
            </div>
            <div className="bg-black/30 border border-white/5 rounded-xl p-3 flex items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                 ...
                 <div>
                   ...
                   <p className="text-[10px] text-brand-400 font-semibold uppercase">{role}</p>
                 </div>
              </div>
          ...
          <nav className="flex-1 space-y-3 relative z-10">
            ...
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-3.5 rounded-xl transition-all duration-300 group ${activeTab === 'dashboard'
                  ? 'bg-gradient-to-r from-brand-600/20 to-transparent border border-brand-500/20 text-brand-400 font-bold shadow-[inset_4px_0_0_rgba(124,82,40,1)]'
                  : isVendedor
            ...
            >
              <LayoutDashboard size={18} className={`${activeTab === 'dashboard' ? 'drop-shadow-[0_0_8px_rgba(124,82,40,0.8)]' : (!isVendedor && 'group-hover:scale-110 transition-transform')}`} />
              <span className="text-sm tracking-wide">Panel de Control</span>
            </button>

            <button
              onClick={() => setActiveTab('panel')}
              className={`w-full flex items-center gap-3 px-4 py-3.5 rounded-xl transition-all duration-300 group ${activeTab === 'panel' ? 'bg-gradient-to-r from-brand-600/20 to-transparent border border-brand-500/20 text-brand-400 font-bold shadow-[inset_4px_0_0_rgba(124,82,40,1)]' : 'border border-transparent text-gray-400 hover:bg-white/5 hover:text-white'
                }`}
            >
              <Store size={18} className={`${activeTab === 'panel' ? 'drop-shadow-[0_0_8px_rgba(124,82,40,0.8)]' : 'group-hover:scale-110 transition-transform'}`} />
            ...
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-3.5 rounded-xl transition-all duration-300 group ${
                activeTab === 'pedidos'
                  ? 'bg-gradient-to-r from-amber-600/20 to-transparent border border-amber-500/20 text-amber-500 font-bold shadow-[inset_4px_0_0_rgba(245,158,11,1)]'
                  : contadorPendientes > 0
                    ? 'border border-transparent text-amber-500/80 hover:bg-amber-500/5 hover:text-amber-500'
                    : 'border border-transparent text-gray-400 hover:bg-white/5 hover:text-white'
              }`}
            >
              ...
              {contadorPendientes > 0 && (
                <div className="ml-auto flex items-center gap-1.5">
                  <span className="text-[10px] font-black text-amber-500">{contadorPendientes}</span>
        ...
        <nav className="md:hidden fixed bottom-6 left-1/2 -translate-x-1/2 w-[90%] max-w-sm bg-[#080d08]/90 backdrop-blur-xl border border-white/10 rounded-2xl flex justify-around p-2 z-50 shadow-[0_20px_40px_rgba(0,0,0,0.8)]">
          <button
            onClick={() => !isVendedor && setActiveTab('dashboard')}
            disabled={isVendedor}
            className={`flex flex-col items-center justify-center flex-1 py-2.5 rounded-xl transition-all duration-300 ${activeTab === 'dashboard' ? 'bg-gradient-to-t from-brand-600/20 to-transparent text-brand-400' : isVendedor ? 'opacity-30 cursor-not-allowed grayscale' : 'text-gray-500 hover:text-gray-300'
              }`}
          >
            <LayoutDashboard size={20} className={activeTab === 'dashboard' ? 'drop-shadow-[0_0_8px_rgba(124,82,40,0.8)] -translate-y-1 transition-transform' : ''} />
            <span className={`text-[10px] uppercase font-bold tracking-wider mt-1.5 ${activeTab === 'dashboard' ? '' : 'scale-90 opacity-50'}`}>Panel</span>
            {activeTab === 'dashboard' && <div className="w-1 h-1 bg-brand-400 rounded-full mt-1 animate-pulse"></div>}
          </button>
          <button
            onClick={() => setActiveTab('panel')}
            className={`flex flex-col items-center justify-center flex-1 py-2.5 rounded-xl transition-all duration-300 ${activeTab === 'panel' ? 'bg-gradient-to-t from-brand-600/20 to-transparent text-brand-400' : 'text-gray-500 hover:text-gray-300'
              }`}
          >
            <Store size={20} className={activeTab === 'panel' ? 'drop-shadow-[0_0_8px_rgba(124,82,40,0.8)] -translate-y-1 transition-transform' : ''} />
            <span className={`text-[10px] uppercase font-bold tracking-wider mt-1.5 ${activeTab === 'panel' ? '' : 'scale-90 opacity-50'}`}>Caja</span>
            {activeTab === 'panel' && <div className="w-1 h-1 bg-brand-400 rounded-full mt-1 animate-pulse"></div>}
          </button>
          <button
            onClick={() => setActiveTab('pedidos')}
            className={`flex flex-col items-center justify-center flex-1 py-2.5 rounded-xl transition-all duration-300 ${
              activeTab === 'pedidos'
                ? 'bg-gradient-to-t from-amber-600/20 to-transparent text-amber-500'
                : contadorPendientes > 0

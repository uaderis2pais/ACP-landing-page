...
export default function Dashboard() {
  ...
  // Totales y Gráficos — construido dinámicamente según config.sucursales
  const sucursalKeys = config.sucursales; // ['Sucursal A', 'Sucursal B', ...]
  const initialTotales = sucursalKeys.reduce((acc, s) => ({ ...acc, [s]: 0 }), { global: 0 });
  ...
  const fetchVentas = async () => {
    try {
      ...
      let gl = 0;
      const sucMap = {}; // monto por sucursal
      const hMap = {};
      const dMap = {
      ...
      ventasCompletadas.forEach(item => {
        ...
        // Acumular por sucursal (dinámico)
        const suc = (item.sucursal || '').toLowerCase().trim();
        config.sucursales.forEach(s => {
          if (suc.includes(s.toLowerCase().split(' ')[1] || s.toLowerCase())) {
            sucMap[s] = (sucMap[s] || 0) + monto;
          }
        });
      ...
      const newTotales = { global: gl };
      config.sucursales.forEach(s => { newTotales[s] = sucMap[s] || 0; });
      ...
      // Torta: Sucursales (dinámico desde config)
      const pieD = config.sucursales
        .map(s => ({ name: s, value: sucMap[s] || 0 }))
        .filter(d => d.value > 0);
    ...
    <div className="p-4 md:p-8 space-y-6 animate-in fade-in zoom-in-95 duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 pb-4 border-b border-white/5 gap-4">
        <div className="flex items-center justify-between md:justify-start gap-4">
           <div>
              <p className="text-[10px] text-brand-400 font-black tracking-widest uppercase opacity-80">Administración — {config.nombreDelNegocio}</p>
           ...
           <div className="md:hidden w-12 h-12 rounded-xl bg-gradient-to-br from-brand-900/50 to-black border border-brand-500/20 shadow-[0_0_15px_rgba(124,82,40,0.2)] flex items-center justify-center">
              🍣
           </div>
        </div>
        ...
        <div className="flex items-center gap-2 bg-black/40 p-1.5 rounded-xl border border-white/10 w-full md:w-auto overflow-hidden">
           {['dia', 'semana', 'mes'].map(t => (
              <button 
                 ...
                 disabled={loading}
                 className={`flex-1 md:flex-none px-4 py-2 text-[10px] sm:text-xs font-bold uppercase tracking-widest rounded-lg transition-all ${timeRange === t ? 'bg-gradient-to-r from-brand-600/30 to-brand-800/30 text-brand-400 shadow-inner' : 'text-gray-500 hover:text-gray-300 hover:bg-white/5 disabled:opacity-50'}`}
              >
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-20 opacity-50 animate-pulse text-brand-500">
      ...
      <>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-gradient-to-br from-brand-500 to-brand-700 rounded-2xl p-6 shadow-[0_0_40px_rgba(124,82,40,0.3)] col-span-1 relative overflow-hidden group">
            ...
            <p className="text-xs font-bold text-brand-100 uppercase tracking-widest mb-3 opacity-90">
          ...
          {/* Cards de sucursales (dinámico) */}
          {config.sucursales.map((suc, idx) => (
            <div key={suc} className="bg-[#0c130d]/80 backdrop-blur-xl border border-white/5 hover:border-brand-500/30 transition-all duration-300 rounded-2xl p-6 flex flex-col justify-center shadow-lg group">
                <p className="text-xs font-bold text-gray-500 flex items-center gap-2 mb-3 uppercase tracking-wider">
                  <MapPin size={14} className="text-brand-500 drop-shadow-[0_0_8px_rgba(124,82,40,0.8)]"/> {suc}
                </p>
                <h3 className="text-3xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-white to-gray-300">${(totales[suc] || 0).toLocaleString()}</h3>
                <div className="h-1.5 bg-black mt-4 rounded-full overflow-hidden shadow-inner w-full">
                   <div className="h-full bg-gradient-to-r from-brand-600 to-brand-400 rounded-full shadow-[0_0_10px_rgba(124,82,40,0.5)]" style={{width: `${totales.global ? ((totales[suc]||0)/totales.global)*100 : 0}%`}}></div>
                </div>
            </div>
          ))}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
          ...
          <div className="bg-[#0c130d]/80 backdrop-blur-xl border border-white/5 rounded-2xl p-6 shadow-lg">
             <div className="flex justify-between items-center mb-8">
                <h4 className="text-xs font-black text-gray-400 uppercase tracking-widest flex items-center gap-2">
                   <Clock size={16} className="text-brand-500" /> Ventas por Hora
                </h4>
             </div>
             ...
             <div className="h-56 w-full">
               <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={hourlyData} margin={{top: 0, right: 0, bottom: 0, left: 0}}>
                     <Tooltip cursor={{fill: 'rgba(255,255,255,0.02)'}} contentStyle={{backgroundColor: 'rgba(10,17,10,0.9)', backdropFilter: 'blur(10px)', border: '1px solid rgba(124,82,40,0.2)', borderRadius: '12px', boxShadow: '0 10px 25px rgba(0,0,0,0.5)'}} itemStyle={{color: '#fff', fontWeight: 'bold'}} labelStyle={{color: '#9ca3af', textTransform: 'uppercase', fontSize: '12px'}} formatter={(value) => [`$${value.toLocaleString()}`, 'Ventas']} />
          ...
          <div className="bg-[#0c130d]/80 backdrop-blur-xl border border-white/5 rounded-2xl p-6 shadow-lg">
             <div className="flex justify-between items-center mb-8">
                <h4 className="text-xs font-black text-gray-400 uppercase tracking-widest flex items-center gap-2">
                   <TrendingUp size={16} className="text-brand-500" /> Tendencia Activa
                </h4>
             </div>
             <div className="h-56 w-full">
               <ResponsiveContainer width="100%" height="100%">
                 <LineChart data={weekData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                   ...
                   <Tooltip contentStyle={{backgroundColor: 'rgba(10,17,10,0.9)', backdropFilter: 'blur(10px)', border: '1px solid rgba(124,82,40,0.2)', borderRadius: '12px'}} itemStyle={{color: '#fff', fontWeight: 'bold'}} labelStyle={{color: '#9ca3af', fontSize: '12px'}} formatter={(value) => [`$${value.toLocaleString()}`, 'Total']} />
                   <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{fill: '#6b7280', fontSize: 11, fontWeight: 600}} dy={15} />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-6">
           ...
           <div className="col-span-1 lg:col-span-1 bg-[#0c130d]/80 backdrop-blur-xl border border-white/5 rounded-2xl p-6 flex flex-col md:flex-row lg:flex-col items-center justify-between shadow-lg">
               <h4 className="text-xs font-black text-gray-400 uppercase tracking-widest flex items-center gap-2 w-full mb-6">
                 <Hash size={16} className="text-brand-500" /> Rendimiento
               </h4>
               <div className="flex lg:flex-col items-center justify-center gap-6 w-full h-full">
                  <div className="w-28 h-28 relative shrink-0 drop-shadow-[0_0_15px_rgba(124,82,40,0.2)]">
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                       <span className="text-[9px] font-black tracking-widest text-brand-400 mb-0.5">MAYOR</span>
           ...
           <div className="col-span-1 lg:col-span-1 bg-[#0c130d]/80 backdrop-blur-xl border border-white/5 rounded-2xl p-6 shadow-lg flex flex-col">
               <div className="flex justify-between items-center mb-6">
                   <h4 className="text-xs font-black text-gray-400 uppercase tracking-widest flex items-center gap-2">
                     <Package size={16} className="text-brand-500" /> Top Productos
                   </h4>
               </div>
                      ...
                      topProducts.map((p, i) => (
                        <div key={i} className="group">
                           <div className="flex justify-between text-xs mb-2">
                              ...
                              <span className="text-brand-400 font-black">{p.qty} uds</span>
                           </div>
                           <div className="h-1.5 bg-black rounded-full overflow-hidden shadow-inner flex-shrink-0">
                              <div className="h-full bg-gradient-to-r from-brand-600 to-brand-400 rounded-full shadow-[0_0_8px_rgba(124,82,40,0.5)] group-hover:opacity-80 transition-opacity" style={{width: `${(p.qty/p.max)*100}%`}}></div>
                           </div>
                        </div>
                      ))
                   )}
           ...
           <div className="col-span-1 lg:col-span-1 bg-[#0c130d]/80 backdrop-blur-xl border border-white/5 rounded-2xl p-6 flex flex-col md:flex-row lg:flex-col items-center justify-between shadow-lg">
               <h4 className="text-xs font-black text-gray-400 uppercase tracking-widest flex items-center gap-2 w-full mb-6">
                 <Hash size={16} className="text-brand-500" /> Canales
               </h4>
               <div className="flex lg:flex-col items-center justify-center gap-6 w-full h-full">
                  <div className="w-28 h-28 relative shrink-0 drop-shadow-[0_0_15px_rgba(124,82,40,0.2)]">
                    <ResponsiveContainer width="100%" height="100%">
        ...
        <div className="bg-[#0c130d]/80 backdrop-blur-xl border border-white/5 rounded-2xl p-6 shadow-lg mt-6">
            <div className="flex justify-between items-center mb-6">
                <h4 className="text-xs font-black text-brand-400 uppercase tracking-widest flex items-center gap-2">
                  <Clock size={16} className="text-brand-500" /> Registro Expandido de Operaciones
                </h4>
            </div>
                           ...
                           <>
                              {paginatedVentas.map((v) => (
                                 <tr key={v.id || Math.random()} className="border-b border-white/5 hover:bg-white/5 transition-colors group">
                                    ...
                                    <td className="px-4 py-3 text-right font-black text-brand-400 drop-shadow-sm">${parseFloat(v.monto_total || 0).toLocaleString()}</td>
                                 </tr>
                              ))}
                              {totalPages > 1 && (
                                 <tr>
                                       ...
                                       <div className="flex items-center justify-between">
                                          ...
                                          <div className="flex gap-2">
                                             <button 
                                                ...
                                                disabled={page === 1}
                                                className="px-3 py-1.5 bg-black/40 border border-white/10 rounded-lg text-xs font-bold text-gray-400 hover:text-white hover:border-brand-500/50 disabled:opacity-30 disabled:hover:border-white/10 transition-all"
                                             >Anterior</button>
                                             <button 
